...
- 按 **听辨回想 → 表达内化 → 口语输出** 的顺序练习，单词、词伙、句型会获得不同的内化提示。
- 每一步都先在口中作答，再显示参考答案，最后由自己标记 **我会** 或 **还不会**。
- 只让已掌握上一阶段的表达继续前进，并把未掌握项目放进本次学习单最后的 **再过一遍** 列表。
- 可选择雅思口语题型与高频主题、职场、日常聊天或旅行场景。练习内容由 DeepSeek 根据原句语境生成。
