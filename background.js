...
chrome.storage.local
  ...
  .catch((error) =>
    console.warn("[WatchToSay] Could not restrict storage access:", error),
  );
  ...
  if (!bundledIeltsBankPromise) {
    bundledIeltsBankPromise = (async () => {
      try {
        ...
        debugLog("[WatchToSay] Optional IELTS bank unavailable:", error);
        return null;
      }
    })();
  }
 ...
 * you to every tab. To make WatchToSay behave like a YouTube-only tool, we
...
async function closePanelForTab(tabId, windowId) {
  ...
  try {
    // This closes the tab-specific panel used by WatchToSay.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "openSidePanel") {
    ...
    debugLog("[WatchToSay BG] openSidePanel requested from tab:", tabId);
    ...
    if (tabId) {
      ...
      chrome.sidePanel
        ...
        .catch((err) => {
          console.error("[WatchToSay BG] openSidePanel error:", err);
        });
    } else {
        ...
        .then((tabs) => {
          if (tabs[0]) {
            ...
            chrome.sidePanel.open({ tabId: tabs[0].id }).catch((err) => {
              console.error(
                "[WatchToSay BG] openSidePanel fallback error:",
                err,
              );
            });
          }
  ...
  if (message.action === "relayToContent") {
    debugLog("[WatchToSay BG] Relay request:", message.payload?.action);
    (async () => {
      try {
        ...
        debugLog(
          "[WatchToSay BG] Active tab in last focused window:",
          tabs.length,
          tabs[0]?.url,
        );
        ...
        if (!tabs[0] || !tabs[0].url?.includes("youtube.com")) {
          ...
          debugLog("[WatchToSay BG] Active YouTube tabs:", tabs.length);
        }

        // Still nothing? Try any YouTube tab
        if (!tabs[0]) {
          tabs = await chrome.tabs.query({ url: "https://www.youtube.com/*" });
          debugLog("[WatchToSay BG] Any YouTube tabs:", tabs.length);
        }

        if (tabs[0]) {
          debugLog(
            "[WatchToSay BG] Sending to tab:",
            tabs[0].id,
            "URL:",
            tabs[0].url,
          );
          ...
          debugLog("[WatchToSay BG] Got response from content:", response);
          sendResponse({ success: true, response });
        } else {
          debugLog("[WatchToSay BG] No YouTube tab found");
          sendResponse({ success: false, error: "No YouTube tab found" });
        }
      } catch (err) {
        console.error("[WatchToSay BG] Relay error:", err.message);
...
async function getPlayerVideoDetails(tabId) {
  try {
    ...
    console.warn("[WatchToSay BG] Player details unavailable:", e.message);
    return null;
  }
}
  ...
  try {
    ...
    if (!settings.supadataApiKey) {
      return {
        ...
        message: "Supadata API key not configured. Open WatchToSay Settings.",
      };
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return {
          ...
          message: "Your Supadata API key is invalid. Open WatchToSay Settings.",
        };
      }
  ...
  try {
    ...
    if (!settings.aiApiKey) {
      return {
        success: false,
        error: "NO_AI_KEY",
        message: "DeepSeek API key not configured. Open WatchToSay Settings.",
      };
    }
    ...
    debugLog("[WatchToSay] Requesting video analysis", settings.aiModel);
  ...
  try {
    ...
    try {
      ...
      if (cached[`digest_${videoId}`]?.transcript) {
        ...
        debugLog("[WatchToSay] Using cached transcript for note");
      }
    } catch (e) {
      debugLog("[WatchToSay] No cached transcript, fetching...");
    }
    ...
    console.error("[WatchToSay] Save note error:", error);
...
async function cleanupNoteText(
  ...
  try {
    debugLog("[WatchToSay] Requesting note cleanup");
    const variables = {
      videoTitle: videoTitle || "Unknown",
      fullContext,
    ...
    try {
      ...
      console.warn(
        "[WatchToSay] JSON parse failed for note, stripping preambles:",
        parseError,
      );
    ...
    console.error("[WatchToSay] Cleanup error:", e);
  }
...
async function handleExplainSelection(
  ...
  try {
    ...
    debugLog("[WatchToSay] Requesting selection explanation");
...
async function handleTranslateContent(
  ...
  try {
    ...
    console.error("[WatchToSay] Translation error:", error);
